...
const subCoachOptions = coachOptions.filter((coach) => coach.value !== selectedMainCoach);
  ...
  const getBatches = () => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/batch-api/get-Tbatches`, {
        headers: { Authorization: "Bearer " + token },
      })
      .then((response) => {
        if (response.status === 200) {
  ...
  const addNewBatch = () => {
    if (Object.keys(errors).length === 0) {
     ...
     console.log(modifiedBatch)
      ...
      axios
        .post(`${process.env.REACT_APP_API_URL}/batch-api/add-Tbatch`, finalBatch, {
          headers: { Authorization: "Bearer " + token },
        })
        .then((response) => {
          if (response.status === 200) {
  ...
  const renderHeader = () => {
    return (
      <div className="flex flex-wrap gap-3 justify-content-between align-items-center">
        <h4 className="m-0">Theory session</h4>
